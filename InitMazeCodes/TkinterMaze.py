...
def check_neighbours(ccr, ccc):
    neighbours = [[
        ...
        # left
        [ccr, ccc + 1, ccr - 1, ccc + 2, ccr, ccc + 2, ccr + 1,
            ccc + 2, ccr - 1, ccc + 1, ccr + 1, ccc + 1],  # right
        [ccr - 1, ccc, ccr - 2, ccc - 1, ccr - 2, ccc, ccr - 2,
         ccc + 1, ccr - 1, ccc - 1, ccr - 1, ccc + 1],  # top
...
def draw_rect():
    ffs.create_rectangle((x1, y1, x1 + width, y1 + height), fill="green")


def del_rect():
    ffs.create_rectangle((x1, y1, x1 + width, y1 + height), fill="white")
